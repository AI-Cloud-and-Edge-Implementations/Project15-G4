...
class AzureDataImporter:
    ...
    def azure_path(self, source_file_path):
        ...
        azure_path = "https://{}/{}/{}/{}".format(
            self.blob_string, self.container_name, source_file_path, self._sas_key
        )
        return azure_path
    ...
    def az_copy_data_from_s3(self, source_file_path, destination_file_path):
        ...
        """
        self._logger.info(
            "Sending file from {} to {}".format(source_file_path, destination_file_path)
        )
        amazon_path = "https://s3.us-west-2.amazonaws.com/congo8khz-pnnn/recordings/wav/{}".format(
            source_file_path
        )
        azure_path = "https://{}/{}/{}/{}".format(
            self.blob_string, self.container_name, destination_file_path, self._sas_key
        )
        command_to_run = [
            "azcopy",
            "--source",
    ...
    def get_unique_file_names(elephant_table):
        ...
        seen = {}
        return [
            seen.setdefault(file, file)
            for file in elephant_table["filename"].tolist()
            if file not in seen
        ]
    ...
    def send_to_copy_handler(self):
        ...
        for dest_path, source_paths in processed_file_map.items():
            self._logger.info(
                "Total Number of files for {} {}".format(
                    dest_path.folder_path, len(source_paths)
                )
            )
